...
generate_encryption_key() {
    ...
    echo -e "${CYAN}============================================${NC}"
    echo -e "${YELLOW}Press Enter after copying the key...${NC}"
    read -p ""
}
...
create_client_config() {
    ...
    # Obfuscation: Derive MTU and TCP flag from the encryption key to ensure client/server match
    local hash_val=$(echo -n "$ENCRYPTION_KEY" | cksum | awk '{print $1}')
    local obfuscated_mtu=$(( (hash_val % 201) + 1200 ))
    
    local flag_idx=$(( hash_val % 3 ))
    local obfuscated_flag
    case $flag_idx in
        0) obfuscated_flag="[\"PA\"]" ;;
        1) obfuscated_flag="[\"A\"]" ;;
        2) obfuscated_flag="[\"PA\", \"A\"]" ;;
    esac
...
network:
  ...
  tcp:
    local_flag: $obfuscated_flag           # Local TCP flags (Obfuscated)
    remote_flag: $obfuscated_flag          # Remote TCP flags (Obfuscated)
...
transport:
  ...
  kcp:
    ...
    mtu: $obfuscated_mtu                    # Maximum transmission unit (Obfuscated)
...
setup_firewall_rules() {
    local port="$1"
    local client_ip="${2:-}"  # Optional: restrict access to client IP only (Active Probing protection)
    ...
    # Rule 0: Active Probing Protection (server only)
    if [ -n "$client_ip" ]; then
        print_info "Rule 0: Active Probing Protection - allowing only $client_ip..."
        # Allow only the specific client IP to reach this port, silently DROP everything else
        iptables -I INPUT -p tcp --dport "$port" -s "$client_ip" -j ACCEPT
        iptables -A INPUT -p tcp --dport "$port" -j DROP
        print_success "Active Probing protection applied (only $client_ip allowed)"
    fi
...
deploy_single_tunnel() {
    ...
    if [ "$MODE" = "server" ]; then
        ...
        setup_firewall_rules "$SERVER_PORT" "$CLIENT_IP"
    else
...
get_single_tunnel_input() {
    ...
    if [ "$MODE" = "client" ]; then
        ...
        # Get server port
        local default_random_port=$(( (RANDOM % 50001) + 10000 ))
        read -p "Enter SERVER port (press Enter for random: $default_random_port): " response
        if [ -n "$response" ]; then
            local normalized
            ...
            if [ -n "$normalized" ]; then
                ...
                print_warning "Invalid port input, using random: $default_random_port"
                SERVER_PORT="$default_random_port"
            fi
        else
            SERVER_PORT="$default_random_port"
        fi
        ...
        # Get server port
        local default_random_port=$(( (RANDOM % 50001) + 10000 ))
        read -p "Enter port to listen on (press Enter for random: $default_random_port): " response
        if [ -n "$response" ]; then
            local normalized
            normalized=$(normalize_port "$response")
            if [ -n "$normalized" ]; then
...
get_multi_client_input() {
    ...
    while [ "$add_more" = "yes" ] || [ "$add_more" = "y" ]; do
        ...
        # Get server port
        local default_random_port=$(( (RANDOM % 50001) + 10000 ))
        local server_port="$default_random_port"
        read -p "Enter SERVER port (default random: $default_random_port): " response
        if [ -n "$response" ]; then
            local normalized
            normalized=$(normalize_port "$response")
            if [ -n "$normalized" ]; then
...
get_multi_server_input() {
    ...
    while [ "$add_more" = "yes" ] || [ "$add_more" = "y" ]; do
        ...
        local server_port=""
        local default_port=$((20000 + tunnel_count - 1))
